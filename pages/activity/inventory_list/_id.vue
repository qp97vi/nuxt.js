...
<template>
	<div>
		...
		<div class="search-box p">
			<div class="w1224">
				<div class="fl" @click="jumpindex" style="width: 100px;height: 100px;">
					<img style="width: 100%;height: 100%;" src="@/static/images/nicefoodlogo.png" alt="">
				</div>
				<div class="search-path fl" style="margin-left:20px ;">
							...
							<li v-for="(item,index) in goodlist" :key="index">
								<router-link target="_blank" class="seckill_mod_goods_link" :to="{name:'goodsInfo',query:{id:item.goods_id}}">
									<div class="s_xsall">
										...
										<div class="price-tag">
											<span class="now"><em class="li_xfo">{{i18nCommon.symbol}}</em><em>{{item.shop_price}}</em></span>
											<!-- <span class="pre-per"><em>{{item.click_count}}</em>{{i18n.promoteList.person_end_browse}}</span> -->
...
<script>
	...
	export default {
		...
		methods: {
			_getInventory() {
				getInventory(this.$route.query.id).then(res => {
					let goodsIds = []
					if(res) {
						res.inventory_goods_list.forEach(ig => {
							goodsIds.push(ig.goods_id)
						})
						this._getGoodsPage(goodsIds.toString());
					}
				})
			},
			_getGoodsPage(goods_ids) {
				getGoods({
					goods_ids: goods_ids,
					p:this.params.page,
