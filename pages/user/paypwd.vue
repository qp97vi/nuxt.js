...
<template>
  <div class="ri-menu fr">
    <div class="menumain">
      <div class="goodpiece">
        <h1>{{i18n.paypwd.security_settings}}</h1>
        <!--<a href=""><span class="co_blue">帮助</span></a>-->
      </div>
      <div class="accouun"></div>
      <div class="thirset ma-to-20">
        <div :class="bool == 0 ? 'wshef yellc' : 'wshef'">
          1.{{i18n.paypwd.authentication}}
          <i class="spassw"></i>
        </div>
        <div :class="bool == 1 ? 'wshef yellc' : 'wshef'">
          2.{{i18n.paypwd.set_paymentpwd}}
          <i class="spassw"></i>
        </div>
        <div :class="bool == 2 ? 'wshef yellc' : 'wshef'">3.{{i18n.paypwd.complete}}</div>
      </div>
      <div class="personerinfro verifyi" v-if="bool == 0">
        <form action method="post">
          <ul class="birth_jz">
            <li class="infor_wi_le">
              <a href="javascript:void(0);">{{i18n.paypwd.verification_mode}}：</a>
            </li>
            <li>
              <select v-model="type" @change="checkType">
                <option v-if="user.mobile" :value="0">{{i18n.paypwd.verification_mobile}}</option>
          ...
          <ul class="name_jz phone sender_name" v-if="type == 0">
            <li class="infor_wi_le">
              <a href="javascript:void(0);">{{i18n.paypwd.phone_number}}：</a>
            </li>
            <li>
          ...
          <ul class="name_jz email sender_name" v-if="type == 1">
            <li class="infor_wi_le">
              <a href="javascript:void(0);">{{i18n.paypwd.mail_number}}：</a>
            </li>
            <li>
          ...
          <ul class="name_jz checode">
            <li class="infor_wi_le">
              <a href="javascript:void(0);">{{i18n.paypwd.auth_code}}：</a>
            </li>
            <li class="teaeu">
          ...
          <ul class="hobby_jz">
            ...
            <div class="save_s">
              <input class="save" type="button" @click="nextstep" :value="i18n.paypwd.next_step" />
            </div>
          </ul>
        </form>
      </div>
      <div class="personerinfro verifyi-next" v-if="bool == 1">
		  
		  <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="120px" class="demo-ruleForm" style="width: 30%;margin: auto;">
		    <el-form-item :label="i18n.paypwd.set_paymentpwd" prop="paypwd">
		      <el-input type="password" v-model="ruleForm.paypwd" size="mini"></el-input>
		    </el-form-item>
		    <el-form-item :label="i18n.paypwd.affirm_paymentpwd" prop="check_paypwd">
		      <el-input type="password" v-model="ruleForm.check_paypwd" size="mini"></el-input>
		    </el-form-item>
		    <el-form-item>
		      <el-button type="danger" @click="nextstep('ruleForm')" :loading="isLoading">{{i18n.paypwd.next_step}}</el-button>
		    </el-form-item>
		  </el-form>
      </div>
...
export default {
  data() {
	var validateNewPwd = (rule, value, callback) => {
	  if (value === "" || value === undefined) {
	    callback(new Error(this.$t('user.paypwd.verify_one')));
	  } else {
	    callback();
	  }
	};
	var validateCheckNewPwd = (rule, value, callback) => {
	  if (value === "" || value === undefined) {
	    callback(new Error(this.$t('user.paypwd.verify_two')));
	  } else if (value !== this.ruleForm.paypwd) {
		callback(new Error(this.$t('user.paypwd.verify_therr')));
	  } else {
	    callback();
	  }
	};
  ...
  computed:{
		i18n () {  
      this.codeText = this.$t('user.paypwd.get_code');
	  	return this.$t('user')  
		}
	},
  methods: {
      ...
      if (this.bool == 1) {
		...
		this.$refs["ruleForm"].validate(valid => {
			if (valid) {
				...
				this.ruleForm.paypwd = md5("TPSHOP" + this.ruleForm.paypwd);
				this.ruleForm.verification = this.code;
				changePaymentPassword(this.ruleForm).then(res => {
					if (res.status == 1) {
						that.bool = 2
					} else {
				...
				that.$message.error(that.i18n.paypwd.parameter_error);
				return false;
			}
		});
      }
