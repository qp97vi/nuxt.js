    ...
    <div class="menumain p">
      <!-- 导航栏 -->
       <el-tabs v-model="activeName" @tab-click="tabClick">
          <el-tab-pane :label="i18n.order_list.all_orders" name=""></el-tab-pane>
          <el-tab-pane :label="i18n.order_list.obligation" :name="0"></el-tab-pane>
          <el-tab-pane label="已付款" name="1,2"></el-tab-pane>
      ...
      <div class="menu_search p">
        <div id="search_order">
          ...
          <button class="sea_et" @click="inquire" :value="year+'/'+(month-3)+'/'+day">{{i18n.order_list.query}}</button>
          <select name="select_year" @change="inquire">
            <option selected :value="year+'/'+(month-3)+'/'+day">{{i18n.order_list.recent_orders}}</option>
      ...
      <div class="order-alone-li lastset_cm" v-if="bool">
        <table width="100%" border cellspacing cellpadding :id="'ordertable_534'+index" v-for="(item,index) in pagingData" :key="item.order_id">
          <tbody>
            <tr class="time_or">
              <td colspan="6">
                <div class="fl_ttmm">
                  ...
                  <span class="time-num">
                    ...
                    <a v-if="isImOn()" href="javascript:;" :user_id="user.user_id" :uname="user.nickname" :avatar="user.head_pic" sign="" :storeid="item.store_id" :client_host="location.host" :client_href="location.href" :goods_id="item.order_goods[0].goods_id" :im_href="location.origin + '/im'" :ws_socket="getWssOrws() + '//' + location.host + '/ws'" :order_id="item.order_id" :order_sn="item.order_sn" :add_time="item.add_time_detail" :full_address="item.address" :consignee="item.consignee + item.mobile" id="workerman-kefu" onclick="jump()">
                      <em class="num">
                        {{item.store_name}}
                        <i class="ear"></i>
                      </em>
            ...
            <tr class="conten_or" v-for="(commit,index) in item.order_goods" :key="commit.rec_id">
              <td class="sx1">
                <div class="shop-if-dif" @click="goods_details(commit)">
...
export default {
  ...
  methods: {
    ...
    inquire(id) {
      ...
      var time = year + "/" + (this.month - 3) + "/" + this.day;
